...
namespace Open.FileExplorer.GoogleDrive
{
    public class GoogleDriveFileSystem : AuthenticatedFileSystem, ISearchExtension
    {
        ...
        private Open.GoogleDrive.File _rootFolder;
        ...
        private FileSystemItem ConvertFile(Open.GoogleDrive.File file, string dirId = null)
        {
        ...
        {
            var collectionId = System.IO.Path.GetFileName(path);
            var client = new GoogleDriveClient(await GetAccessTokenAsync(true, cancellationToken));
            var dirId = System.IO.Path.GetFileName(path);
            if (string.IsNullOrWhiteSpace(dirId))
                dirId = Root;
            var file = new Open.GoogleDrive.File
            {
                Name = item.Name,
        ...
        {
            var newFolder = new Open.GoogleDrive.File
            {
                Name = item.Name,
        ...
        protected override async Task<FileSystemFile> CopyFileAsyncOverride(string sourceFileId, string targetDirId, FileSystemFile file, CancellationToken cancellationToken)
        {
            ...
            var client = new GoogleDriveClient(await GetAccessTokenAsync(true, cancellationToken));
            var newFile = new Open.GoogleDrive.File { Parents = new string[] { targetDirId } };
            var result = await client.CopyFileAsync(System.IO.Path.GetFileName(sourceFileId), newFile, _fileFields, cancellationToken);
        ...
        {
            var newDoc = new Open.GoogleDrive.File { Name = item.Name };
            var client = new GoogleDriveClient(await GetAccessTokenAsync(true, cancellationToken));
        ...
        protected override async Task<FileSystemDirectory> DeleteDirectoryAsyncOverride(string dirId, bool sendToTrash, CancellationToken cancellationToken)
        {
            ...
            if (sendToTrash)
            {
                var updatedFile = new Open.GoogleDrive.File { Trashed = true };
        ...
        protected override async Task<FileSystemFile> DeleteFileAsyncOverride(string fileId, bool sendToTrash, CancellationToken cancellationToken)
        {
            ...
            {
                await client.DeleteFileAsync(System.IO.Path.GetFileName(fileId), cancellationToken);
                return null;
            }
        }
        ...
        {
            dirId = FileSystemAsync.Path.NormalizePath(dirId);
            var parentId = await GetDirectoryParentIdAsync(dirId, cancellationToken);
        ...
        {
            fileId = FileSystemAsync.Path.NormalizePath(fileId);
            var parentId = await GetFileParentIdAsync(fileId, cancellationToken);
        ...
        {
            var client = new GoogleDriveClient(await GetAccessTokenAsync(true, cancellationToken));
            var updatedFile = new Open.GoogleDrive.File { Trashed = false };
